/**
 * GridMe 共有グリッドページ (FIXED VERSION)
 * 
 * 共有されたテーマグリッドを表示するページ
 * This version fixes the share URL length issue
 */
...
(function() {
    ...
    // Constants
    const MAX_URL_LENGTH = 2048; // Safe URL length for most browsers
    const MAX_IMAGE_SIZE = 50000; // Max size per image in characters
    ...
    const state = {
        ...
        nickname: '', // ニックネームを保存
        imagesWereRemoved: false // Flag to track if images were removed from share
    };
    ...
    function getSharedData() {
        ...
        try {
            ...
            // Check if images were removed during sharing
            if (data.imagesRemoved) {
                state.imagesWereRemoved = true;
                showToast('画像は共有されませんでした。必要に応じて追加してください。', 'info');
            }
    ...
    function initializeGrid() {
        ...
        // Load images if available (backward compatibility)
    ...
    // 画像圧縮関数 (Enhanced for better compression)
    function compressImage(file, callback) {
        ...
        reader.onload = function(e) {
            ...
            img.onload = function() {
                ...
                // 最大サイズを設定（共有用により小さくする）
                const maxSize = 200; // Reduced from 300 for better compression
                let width = img.width;
    ...
    function showShareModal() {
        ...
        if (Object.keys(state.uploadedImages).length > 0) {
            shareData.images = state.uploadedImages;
        }
        
        // Generate and test URL
        let encodedData = encodeShareData(shareData);
        let baseUrl = window.location.origin + window.location.pathname.replace('shared.html', 'theme-grid.html');
        let shareUrl = `${baseUrl}?data=${encodeURIComponent(encodedData)}`;
        
        // Check URL length
        if (shareUrl.length > MAX_URL_LENGTH && shareData.images) {
            // Try without images
            delete shareData.images;
            shareData.imagesRemoved = true;
            encodedData = encodeShareData(shareData);
            shareUrl = `${baseUrl}?data=${encodeURIComponent(encodedData)}`;
            
            // Show warning
            showToast('URLが長すぎるため、画像なしで共有されます', 'warning');
        }
        
        // Still too long? Show error
        if (shareUrl.length > MAX_URL_LENGTH * 2) {
            showToast('共有データが大きすぎます。セクションを減らしてください。', 'error');
